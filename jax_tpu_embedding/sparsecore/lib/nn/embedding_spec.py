...
class OptimizerSpec(metaclass=abc.ABCMeta):
  ...
  def __eq__(self, other: object) -> bool:
    if not isinstance(other, OptimizerSpec):
      return False
...
SlotShape = slot_shape.SlotShape
...
class CustomOptimizerSpec(OptimizerSpec):
  ...
  Attributes:
    ...
    stablehlo: The StableHLO module for the custom optimizer.
    ...
    slot_shapes: Optional shape specification per slot variable. Defaults to
      table-shaped slots. See `slot_shape.SlotShape`.
  """
  ...
  def slot_variables_shapes(
    ...
    for s in self.slot_shapes:
      if s == slot_shape.SlotShape.ROWWISE_1D:
        shapes.append((table_shape[0],))
      elif s == slot_shape.SlotShape.TABLE:
...
class FeatureSpec:
  ...
  Notes:
    FeatureSpec is virtually immutable (for :py:func:`jax.jit`) using
    ``eq=True`` and ``unsafe_hash=True``, but has ``frozen=False`` to
    allow in-place updates when preparing for feature stacking or table
    stacking.
    See [dataclass
    doc](https://docs.python.org/3/library/dataclasses.html#dataclasses.dataclass)
    for more information.
