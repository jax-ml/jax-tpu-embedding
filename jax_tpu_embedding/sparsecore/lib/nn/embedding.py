...
"""Embedding layer API for SparseCore.

This file contains functions for embedding lookup and update on SparseCore,
including input preprocessing, forward/backward passes, and embedding table
...
def compute_row_pointers_size_per_device(
    *,
    ...
    tpu_vector_alignment: int | None = None,
) -> int:
  ...
  if tpu_vector_alignment is None:
    tpu_vector_alignment = utils.tpu_vector_alignment()
...
def compute_coo_buffer_size_per_device(
  ...
  Args:
    ...
    tpu_vector_alignment: The TPU vector alignment size (e.g. 8 for v5, 16 for
      v6).
...
def eval_preprocess_sparse_dense_matmul_input_shape(
  ...
  # NOTE: We reuse the type here instead of re-defining intermediate types with
  # leaf nodes as ShapeDtypeStruct. User will have to use `typing.cast` for the
  # leaf nodes.
