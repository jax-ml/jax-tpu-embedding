...
class TableStackingTest(parameterized.TestCase):
  ...
  def test_stacking_and_sharding_table(self, donate):
    vocab_size_a = 64
    vocab_size_b = 192
    embedding_dim = 14
    batch_size = 16
    ...
    stacked_tables = table_stacking.stack_and_shard_feature_tables(
        feature_tables, table_spec_proto, donate=donate
    )
    ...
    # validate if inputs are donated accordingly
    for tbl in feature_tables.values():
      self.assertEqual(tbl.is_deleted(), donate)
