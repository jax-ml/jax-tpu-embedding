...
:tf_sparse_tensor_input_preprocessing_benchmarks

Or locally:
bazel run --config=benchmark
:tf_sparse_tensor_input_preprocessing_benchmarks -- --benchmark_filter=all
...
def generate_sparse_tensors_for_feature_spec(feature_specs, num_samples):
...
_GLOBAL_RAGGED_INDICES, _GLOBAL_RAGGED_VALUES, _GLOBAL_RAGGED_DENSE_SHAPES = (
    generate_sparse_tensors_for_feature_spec(_GLOBAL_SPECS, num_samples=16000)
)
...
def preprocess_input_benchmark_ragged_tensor_jit(state):
  ...
  while state:
    _ = pybind_input_preprocessing.PreprocessSparseDenseMatmulInputSparseTensor(
