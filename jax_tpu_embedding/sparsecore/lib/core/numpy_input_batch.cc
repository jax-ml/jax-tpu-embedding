...
void _ExtractCooTensors(const py::array& features,
...
void NumpySparseInputBatch::ExtractCooTensors(
  ...
  DCHECK(!PyGILState_Check());  // Does not require external GIL
  py::gil_scoped_acquire _;
  _ExtractCooTensors(feature_, weights_, row_offset, col_offset, col_shift,
                     num_scs, global_device_count, combiner, coo_tensors);
}
