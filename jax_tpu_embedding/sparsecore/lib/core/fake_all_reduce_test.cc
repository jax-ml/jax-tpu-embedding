...
#include "tsl/platform/status_matchers.h"  // from @tsl
...
TEST(FakeAllReduceTest, SingleHostSingleGroup) {
  ...
  // Act & Assert
  EXPECT_THAT(all_reduce(sync_key, 1ULL), IsOkAndHolds(1ULL));
  EXPECT_THAT(all_reduce(sync_key, 0ULL), IsOkAndHolds(0ULL));
}
...
TEST(FakeAllReduceTest, SingleHostMultiGroup) {
  ...
  // Act & Assert
  EXPECT_THAT(all_reduce(0, 1ULL), IsOkAndHolds(1ULL));
  EXPECT_THAT(all_reduce(1, 0ULL), IsOkAndHolds(0ULL));
  EXPECT_THAT(all_reduce(2, 1ULL), IsOkAndHolds(1ULL));
}
...
TEST(FakeAllReduceTest, MultiHostAllOnes) {
  ...
  // Act
  {
    ...
    for (int i = 0; i < host_count; ++i) {
      pool.Schedule([&, i]() {
        TF_ASSERT_OK_AND_ASSIGN(results[i], all_reduce(sync_key, 1ULL));
      });
    }
...
TEST(FakeAllReduceTest, MultiHostMixedValues) {
  ...
  // Act
  {
    ...
    for (int i = 0; i < host_count; ++i) {
      pool.Schedule([&, i]() {
        TF_ASSERT_OK_AND_ASSIGN(results[i],
                                all_reduce(sync_key, i % 2 == 0 ? 1ULL : 0ULL));
      });
    }
    // Destructor waits for completion.
  }
...
TEST(FakeAllReduceTest, MultiHostAllZeros) {
  ...
  // Act
  {
    ...
    for (int i = 0; i < host_count; ++i) {
      pool.Schedule([&, i]() {
        TF_ASSERT_OK_AND_ASSIGN(results[i], all_reduce(sync_key, 0ULL));
      });
    }
    // Destructor waits for completion.
  }
...
TEST(FakeAllReduceTest, MultiHostMultiGroup) {
  ...
  // Act
  {
    ...
    // Group 0
    pool.Schedule(
        [&]() { TF_ASSERT_OK_AND_ASSIGN(results[0], all_reduce(0, 1ULL)); });
    pool.Schedule(
        [&]() { TF_ASSERT_OK_AND_ASSIGN(results[1], all_reduce(0, 0ULL)); });

    // Group 1
    pool.Schedule(
        [&]() { TF_ASSERT_OK_AND_ASSIGN(results[2], all_reduce(1, 0ULL)); });
    pool.Schedule(
        [&]() { TF_ASSERT_OK_AND_ASSIGN(results[3], all_reduce(1, 0ULL)); });
    // Destructor waits for completion.
  }

  // Assert
  EXPECT_THAT(results[0], 1ULL);
  EXPECT_THAT(results[1], 1ULL);
  EXPECT_THAT(results[2], 0ULL);
  EXPECT_THAT(results[3], 0ULL);
}
