...
class RaggedTensorInputBatchTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    embedding_ids_span_ = embedding_ids_;
    embedding_splits_span_ = embedding_splits_;
  }

  std::vector<int64_t> embedding_ids_;
  std::vector<int32_t> embedding_splits_;
  absl::Span<const int64_t> embedding_ids_span_;
  absl::Span<const int32_t> embedding_splits_span_;
};

TEST_F(RaggedTensorInputBatchTest, SliceTest) {
  RaggedTensorInputBatch ragged_tensor_input_batch(embedding_ids_span_,
                                                   embedding_splits_span_);
