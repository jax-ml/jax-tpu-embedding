...
def _tpu_sparse_dense_matmul_optimizer_grad_lowering(
  ...
  # Tagging the update as a SparseCore computation is what lets slot variables
  # have a different shape than the table (e.g. a row-wise Adagrad accumulator):
  # it keeps XLA's main-thread pipeline from rewriting the computation into a
  # uniformly shaped one before the optimizer-update decomposer runs.
  #
  # The cost is that the main-thread inliner no longer runs on it either, and
  # the decomposer does not recurse into called computations. JAX emits private
  # helper `func.func`s for the nested jits inside `jnp.clip`, `jnp.where`, etc,
  # so inline those calls by hand first. jaxlib does not register MLIR's inliner
  # extensions, so the `inline` pass is not available here.
      ...
      for block in func_op.regions[0].blocks:
        for op in list(block.operations):
          if op.operation.name != "func.call":
            continue
          callee_block = (
              sym_tab[op.attributes["callee"].value].regions[0].blocks[0]
          )
          ...
          for inner_op in callee_block.operations:
            if inner_op.operation.name == "func.return":
              for old_res, operand in zip(op.results, inner_op.operands):
                old_res.replace_all_uses_with(val_map.get(operand, operand))
            else:
