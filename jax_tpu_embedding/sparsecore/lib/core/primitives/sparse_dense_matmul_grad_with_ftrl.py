...
def _tpu_sparse_dense_matmul_grad_with_ftrl_lowering(
  ...
  optimizer_update = func_dialect.FuncOp(
      ...
      (
          [
              ...
              ir.RankedTensorType.get([1, emb_dim], f32),  # l1_param_ (γ1)
              ir.RankedTensorType.get([1, emb_dim], f32),  # l2_param_ (γ2)
  ...
  with ir.InsertionPoint(entry_block):
    ...
    accumulator_new_ = hlo.add(accumulator_, grad_square_)
    ...
    # L_n = L_o + g
    linear_plus_grad_ = hlo.add(linear_, grad_)
    linear_new_ = hlo.subtract(linear_plus_grad_, term_to_subtract_linear_)

    # Proximal update part for Ē_n
    # Numerator for weight update: min(|L_n|, γ_1) * sign(L_n) - L_n
    # This simplifies to: (sign(L_n) * γ_1 - L_n) if |L_n| > γ_1, else 0.
    # The 'select' op later will handle the 'else 0' part.
    sign_linear_new_ = hlo.sign(linear_new_)
    # sign_linear_new_times_l1_ = sign(L_n) * γ_1
    ...
    # Denominator for weight update: (1/λ)(P_n + β) + 2γ_2
    # p_n_plus_beta_ = P_n + β
    two_ = hlo_f32_fn(2.0)
    # two_times_l2_ = 2 * γ2
    two_times_l2_ = hlo.multiply(two_, l2_param_)

    # denominator_ = (P_n + β)/λ + 2γ_2
    denominator_ = hlo.add(term1_denominator_, two_times_l2_)

    # Condition for update: |L_n| > γ_1
