...
class SparseDenseMatmulGradWithftrlTest(parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      ...
      dict(
          ...
          learning_rate=learning_rate,
          learning_rate_power=learning_rate_power,
          l1_regularization_strength=1,  # int
          l2_regularization_strength=l2_regularization_strength,
          beta=beta,
          clip_weight_min=clip_weight_min,
          clip_weight_max=clip_weight_max,
      ...
      dict(
          ...
          learning_rate_power=learning_rate_power,
          l1_regularization_strength=l1_regularization_strength,
          l2_regularization_strength=1,  # int
          beta=beta,
          clip_weight_min=clip_weight_min,
          clip_weight_max=clip_weight_max,
          max_ids_per_partition=max_ids_per_partition,
      ...
      dict(
          ...
          beta=beta,
          clip_weight_min=clip_weight_min,
          clip_weight_max=1,  # int
          max_ids_per_partition=max_ids_per_partition,
          max_unique_ids_per_partition=max_unique_ids_per_partition,
      ),
      dict(
          testcase_name="row_pointers_shape_rank_is_not_1",
          row_pointers=np.array([[0, 1, 2, 3]]),  # 2D
          sample_ids=sample_ids,
          embedding_ids=embedding_ids,
          gains=gains,
          embedding_table=embedding_table,
      ...
      dict(
          ...
          embedding_ids=embedding_ids,
          gains=gains,
          embedding_table=np.array([[1.0, 2.0, 3.0, 4.0]]),  # Different shape
          accumulator=accumulator,  # Original shape
          linear=linear,
          activations_grad=activations_grad,
          learning_rate=learning_rate,
          learning_rate_power=learning_rate_power,
      ...
      dict(
          ...
          embedding_ids=embedding_ids,
          gains=gains,
          embedding_table=np.array([1.0, 2.0, 3.0, 4.0]),  # 1D
          accumulator=np.array([1.0, 2.0, 3.0, 4.0]),
          linear=np.array([0.1, 0.2, 0.3, 0.4]),
          activations_grad=activations_grad,  # 2D, will cause mismatch later
          learning_rate=learning_rate,
          learning_rate_power=learning_rate_power,
          l1_regularization_strength=l1_regularization_strength,
          l2_regularization_strength=l2_regularization_strength,
      ...
      dict(
          ...
          accumulator=accumulator,
          linear=linear,
          activations_grad=np.array([1.0, 2.0, 3.0]),  # 1D
          learning_rate=learning_rate,
          learning_rate_power=learning_rate_power,
          l1_regularization_strength=l1_regularization_strength,
          l2_regularization_strength=l2_regularization_strength,
      ...
      dict(
          ...
          embedding_ids=embedding_ids,
          gains=gains,
          embedding_table=np.array([[1.0, 2.0, 3.0]]),  # Width 3
          accumulator=np.array([[1.0, 2.0, 3.0]]),
          linear=np.array([[0.1, 0.2, 0.3]]),
          activations_grad=np.array([[1.0, 2.0, 3.0, 4.0]]),  # Width 4
          learning_rate=learning_rate,
          learning_rate_power=learning_rate_power,
          l1_regularization_strength=l1_regularization_strength,
          l2_regularization_strength=l2_regularization_strength,
      ...
      dict(
          ...
          clip_weight_min=clip_weight_min,
          clip_weight_max=clip_weight_max,
          max_ids_per_partition=0,  # Invalid
          max_unique_ids_per_partition=max_unique_ids_per_partition,
      ),
      dict(
          testcase_name=(
          ...
          clip_weight_max=clip_weight_max,
          max_ids_per_partition=max_ids_per_partition,
          max_unique_ids_per_partition=0,  # Invalid
      ),
  )
  ...
  def _compute_ftrl(
    ...
    k: lr_power_k (learning_rate_power, used as exponent's negative: Ā_α⁻ᵏ)
    ...
    Args:
      ...
      lr_power_k: Learning rate power, used as exponent's negative: Ā_α⁻ᵏ.
    ...
    new_accum_a_n = old_accum_a_o + g_squared  # Ā_n

    # Power terms: P_α = Ā_α⁻ᵏ
    # neg_lr_power_k is -k from the formula Ā_α⁻ᵏ
    ...
    new_linear_l_n = old_linear_l_o + grad_g - term_to_subtract  # L_n

    # Proximal update part for Ē_n
    # Numerator for weight update: (sign(L_n)γ₁ - L_n)
    ...
    term1_denominator = p_n_plus_beta * one_over_lambda  # (P_n + β) / λ
    ...
    # Initialize new table with zeros
  ...
  def _compute_table_grad(
    ...
    # Assume cols are valid indices for the table_grad for test purposes
