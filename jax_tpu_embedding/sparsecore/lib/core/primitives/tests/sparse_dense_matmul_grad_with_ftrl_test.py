...
class SparseDenseMatmulGradWithftrlTest(parameterized.TestCase):
  ...
  weight_decay_factor = np.float32(0.0005)
  multiply_weight_decay_factor_by_learning_rate = np.bool(False)
  multiply_linear_by_learning_rate = np.bool(False)
  allow_zero_accumulator = np.bool(False)
  ...
  _BASE_TEST_PARAMS = dict(
      ...
      allow_zero_accumulator=allow_zero_accumulator,
      weight_decay_factor=weight_decay_factor,
      multiply_weight_decay_factor_by_learning_rate=multiply_weight_decay_factor_by_learning_rate,
  )
  ...
  @parameterized.named_parameters(
      ...
      dict(
          ...
          **{**_BASE_TEST_PARAMS, "clip_weight_min": 1},
      ),
      dict(
          testcase_name="clip_weight_max_is_not_np.float32",
          **{**_BASE_TEST_PARAMS, "clip_weight_max": 1},
      ),
      dict(
      ...
      dict(
          testcase_name="allow_zero_accumulator_is_not_np.float32",
          **{**_BASE_TEST_PARAMS, "allow_zero_accumulator": 1},
      ),
      dict(
  ...
  def _compute_ftrl(
      ...
      weight_decay_factor_val: np.float32,
      mul_wd_by_lr_flag: np.bool,
      mlr_flag: np.bool,
      allow_zero_flag: np.bool,
    ...
    Args:
      ...
      weight_decay_factor_val: Weight decay factor.
      mul_wd_by_lr_flag: Whether to multiply weight decay by learning rate.
      mlr_flag: Whether to use multiply linear by learning rate.
      allow_zero_flag: Whether to allow zero accumulator.
    ...
    zero = np.float32(0.0)
    old_accum_a_o_safe = (
        old_accum_a_o if allow_zero_flag else old_accum_a_o + eps
    )

    # Weight Decay
    wd_scale = (
        weight_decay_factor_val * lr_lambda
        if mul_wd_by_lr_flag
        else weight_decay_factor_val
    )
    wd_term = wd_scale * old_table_e_o
    grad_g = grad_g + wd_term

    # Accumulator
    a_new = old_accum_a_o_safe + grad_g * grad_g

    # Power‑law terms
    p_old = jnp.power(old_accum_a_o_safe, -lr_power_k)
    ...
    if mlr_flag:
      ...
      l_threshold = lr_lambda * l1_gamma1
    else:
      l_new = old_linear_l_o + grad_g - (delta_p / lr_lambda) * old_table_e_o
      l_new_scaled = l_new
      l_threshold = l1_gamma1

    # Proximal weight update
    abs_l = jnp.abs(l_new_scaled)
    sign_l = jnp.sign(l_new_scaled)
    numerator = sign_l * l_threshold - l_new_scaled
    denom = (p_new + beta_b) / lr_lambda + (two * l2_gamma2)
    w_new = jnp.where(abs_l > l_threshold, numerator / denom, zero)
    w_new = jnp.clip(w_new, clip_weight_min_val, clip_weight_max_val)

    return (
        np.asarray(w_new, dtype=np.float32),
        np.asarray(a_new, dtype=np.float32),
        np.asarray(l_new_scaled, dtype=np.float32),
    )
  ...
  def test_ftrl_optimizer_update(self):
    ...
    weight_decay_factor_val = np.float32(0.0005)
    multiply_weight_decay_factor_by_learning_rate_flag = np.bool(False)
    multiply_linear_by_learning_rate_flag = np.bool(False)
    allow_zero_accumulator_flag = np.bool(False)
    ...
    for row_idx in sparse_rows_to_update:
      (
          ...
          clip_min_val,
          clip_max_val,
          weight_decay_factor_val,
          multiply_weight_decay_factor_by_learning_rate_flag,
          multiply_linear_by_learning_rate_flag,
          allow_zero_accumulator_flag,
      )
