...
class SparseDenseMatmulGradWithSgdTest(absltest.TestCase):
  ...
  def test_sc_emb_backward_pass_with_bounds(self):
    # Process the input.
    mesh = jax.sharding.Mesh(self.global_devices, "x")
    (
        lhs_row_pointers,
    ...
    sparse_rows = jnp.unique(jnp.concatenate(np.unstack(self.input_tensor)))
    ...
    expected_emb_activations = jnp.where(
        sparse_update_mask_sharded[0],
        jnp.clip(expected_emb_activations, 2.0, 12.0),
        emb_table_sharded[0],
    )

    np.testing.assert_allclose(expected_emb_activations, updated_emb_table)
