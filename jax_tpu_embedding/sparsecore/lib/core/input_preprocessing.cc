...
//
// TODO: abseil.io/tips/176 - move output and stats buffers to return value.
void PreprocessInputForStackedTablePerLocalDevice(
    ...
    const SparseInput& sparse_input, Eigen::Ref<VectorXi> row_pointer_buffer,
    Eigen::Ref<VectorXi> embedding_id_buffer,
    Eigen::Ref<VectorXi> sample_id_buffer, Eigen::Ref<VectorXf> gain_buffer,
    Eigen::Ref<VectorXi> max_ids_buffer,
    Eigen::Ref<VectorXi> max_unique_ids_buffer,
    Eigen::Ref<VectorXi> required_buffer_size_per_sc_buffer) {
  const int num_scs = options.num_sc_per_device * options.global_device_count;
  ...
  std::vector<CooFormat> coo_tensors =
      sparse_input.ExtractCooTensorsForAllFeatures(
          stacked_table_metadata, local_device_id, options.local_device_count,
          num_scs, options.global_device_count, batch_size_for_device,
...
PreprocessSparseDenseMatmulOutput PreprocessSparseDenseMatmulInput(
    const SparseInput& sparse_input,
    const PreprocessSparseDenseMatmulInputOptions& options) {
  ...
  const absl::flat_hash_map<std::string, std::vector<StackedTableMetadata>>
      stacked_tables = sparse_input.GetStackedTableMetadata();
    ...
    for (const auto& [stacked_table_name, stacked_table_metadata] :
      ...
      PreprocessingThreadPool()->Schedule([&, context_id =
        ...
        for (int local_device = 0; local_device < options.local_device_count;
          ...
          Eigen::Ref<VectorXi> row_pointer_buffer =
              row_pointers_per_device.row(local_device);
          Eigen::Ref<VectorXi> embedding_id_buffer =
              embedding_ids_per_device.row(local_device);
          Eigen::Ref<VectorXi> sample_id_buffer =
              sample_ids_per_device.row(local_device);
          Eigen::Ref<VectorXf> gain_buffer = gains_per_device.row(local_device);
          Eigen::Ref<VectorXi> max_ids_per_partition_per_sc_buffer =
              max_ids_per_partition_per_sc.row(local_device);
          Eigen::Ref<VectorXi> max_unique_ids_per_partition_per_sc_buffer =
          ...
          PreprocessInputForStackedTablePerLocalDevice(
              local_device, stacked_table_metadata, stacked_table_name, options,
              coo_buffer_size_per_device, row_pointers_size_per_sc,
              sparse_input, row_pointer_buffer, embedding_id_buffer,
        ...
        out.lhs_row_pointers[stacked_table_name] =
            std::move(row_pointers_per_device);
        out.lhs_embedding_ids[stacked_table_name] =
            std::move(embedding_ids_per_device);
        out.lhs_sample_ids[stacked_table_name] =
            std::move(sample_ids_per_device);
        out.lhs_gains[stacked_table_name] = std::move(gains_per_device);
        out.stats.max_ids_per_partition[stacked_table_name] =
            std::move(max_ids_per_partition_per_sc);
        out.stats.max_unique_ids_per_partition[stacked_table_name] =
            std::move(max_unique_ids_per_partition_per_sc);
        out.stats.required_buffer_sizes[stacked_table_name] =
