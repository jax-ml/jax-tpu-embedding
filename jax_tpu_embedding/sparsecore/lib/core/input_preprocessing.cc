...
PreprocessSparseDenseMatmulInput(
  ...
  //   1. COO extraction and initial sort/group.
  //   2. Optional re-sort/group based on global minibatching required.
  //   3. Fill row pointers.
  ...
  struct TableState {
    ...
    TableState(const std::string& name,
               const std::vector<StackedTableMetadata>* metadata,
               const PreprocessSparseDenseMatmulInputOptions& options,
  ...
  std::vector<std::unique_ptr<TableState>> table_states;
  for (const auto& [stacked_table_name, stacked_table_metadata] :
       stacked_tables) {
    table_states.push_back(std::make_unique<TableState>(
        stacked_table_name, &stacked_table_metadata, options, num_scs,
        row_pointers_size_per_bucket));
  }

  // Stage 1: COO Extraction and Initial Sort/Group
  absl::BlockingCounter counter1(stacked_tables.size());
  for (auto& state : table_states) {
    PreprocessingThreadPool()->Schedule([&, &state = *state,
                                         context_id = producer.GetContextId()] {
      tsl::profiler::TraceMeConsumer consumer(
          [&] {
            return absl::StrCat("InputPreprocessingTable-S1-",
      ...
      for (int local_device = 0; local_device < options.local_device_count;
        ...
        const PartitionedCooTensors grouped_coo_tensors =
            SortAndGroupCooTensorsPerLocalDevice(
                extracted_coo_tensors, (*state.stacked_table_metadata)[0],
  ...
  if (options.enable_minibatching) {
    ...
    if (options.all_reduce_interface != nullptr) {
      auto reduced_value_or = options.all_reduce_interface->BlockingAllReduce(
          options.batch_number * 2,
          internal::Serialize(local_minibatching_required));
      if (!reduced_value_or.ok()) {
        return reduced_value_or.status();
      }
      global_minibatching_required =
          internal::Deserialize(reduced_value_or.value());
    } else {
      global_minibatching_required = local_minibatching_required;
    }
  }
  ...
  if (options.enable_minibatching && global_minibatching_required) {
    absl::BlockingCounter counter2(stacked_tables.size());
    for (auto& state : table_states) {
      PreprocessingThreadPool()->Schedule(
          [&, &state = *state, context_id = producer.GetContextId()] {
            tsl::profiler::TraceMeConsumer consumer(
                [&] {
                  return absl::StrCat("InputPreprocessingTable-S2-",
                                      state.stacked_table_name);
                },
                context_id);
            state.stats_per_host.dropped_id_count = 0;
            for (int local_device = 0;
                 local_device < options.local_device_count; ++local_device) {
              internal::StatsPerDevice stats_per_device =
                  state.stats_per_host.GetStatsPerDevice(local_device);
              state.partitioned_coo_tensors_per_device[local_device] =
                  SortAndGroupCooTensorsPerLocalDevice(
                      state.extracted_coo_tensors_per_device[local_device],
                      (*state.stacked_table_metadata)[0], options,
                      stats_per_device, state.table_minibatching_split);
              state.stats_per_host.dropped_id_count +=
                  stats_per_device.dropped_id_count;
            }
    ...
    if (options.all_reduce_interface != nullptr) {
      ...
      global_minibatching_split = local_minibatching_split;
    }
  }

  // Stage 3: Fill Device Buffers
  absl::BlockingCounter counter3(stacked_tables.size());
  for (auto& state : table_states) {
    PreprocessingThreadPool()->Schedule([&, &state = *state,
                                         global_minibatching_required,
                                         global_minibatching_split,
      ...
      tsl::profiler::TraceMeConsumer consumer(
          [&] {
            return absl::StrCat("InputPreprocessingTable-S3-",
                                state.stacked_table_name);
          },
          context_id);
      ...
      state.stats_per_host.Flatten();
      {
        ...
        out.lhs_row_pointers[state.stacked_table_name.c_str()] =
            std::move(state.csr_arrays_per_host.row_pointers);
        out.lhs_embedding_ids[state.stacked_table_name.c_str()] =
            std::move(state.csr_arrays_per_host.embedding_ids);
        out.lhs_sample_ids[state.stacked_table_name.c_str()] =
            std::move(state.csr_arrays_per_host.sample_ids);
        out.lhs_gains[state.stacked_table_name.c_str()] =
            std::move(state.csr_arrays_per_host.gains);

        out.stats.max_ids_per_partition[state.stacked_table_name.c_str()] =
            std::move(state.stats_per_host.max_ids_per_partition);
        out.stats
            .max_unique_ids_per_partition[state.stacked_table_name.c_str()] =
            std::move(state.stats_per_host.max_unique_ids_per_partition);
        out.stats.required_buffer_sizes[state.stacked_table_name.c_str()] =
            std::move(state.stats_per_host.required_buffer_size);
        out.stats.dropped_id_count[state.stacked_table_name.c_str()] =
            state.stats_per_host.dropped_id_count;
      }
