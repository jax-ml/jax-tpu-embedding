...
PreprocessSparseDenseMatmulOutput PreprocessSparseDenseMatmulInput(
  ...
  CHECK(options.enable_minibatching || !options.static_single_minibatch);
  ...
  for (const auto& [stacked_table_name, stacked_table_metadata] :
    ...
    PreprocessingThreadPool()->Schedule([&,
      ...
      if (options.enable_minibatching) {
        if (!options.static_single_minibatch) {
        ...
        if (!options.static_single_minibatch &&
            minibatching_req_barrier->Block())
        ...
        if (minibatching_required) {
          ...
          if (!options.static_single_minibatch &&
              minibatching_split_barrier->Block())
  ...
  DCHECK(!options.static_single_minibatch || out.num_minibatches == 1);
