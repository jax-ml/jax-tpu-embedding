...
struct CooFormat {
  ...
  using HashFn = std::function<uint32_t(int)>;
  ...
  uint32_t GetBucketId(HashFn hash_fn = std::identity()) const {
    static_assert((kMaxMinibatchingBuckets & (kMaxMinibatchingBuckets - 1)) ==
                  0);
    return static_cast<uint32_t>(hash_fn(col_id)) &
           (kMaxMinibatchingBuckets - 1);
  }

  // Computes a grouping key for the given `CooFormat` object.
  // The key is used to group and sort COO tensors for efficient processing.
  // The output is of the format [bucket id, shard id, local embedding id,
  // index].
  // TODO: b/428790659 - Update hash function.
  uint64_t GetGroupingKey(const uint32_t num_scs_bit, const int index,
                          const bool enable_minibatching = false,
                          const HashFn hash_fn = std::identity()) const {
    ...
    uint64_t result = static_cast<uint64_t>(bucket_id);
    // Shift bucket_id to the most significant bits of the upper 32-bit part.
    result <<= 32;
    // Combine with the rotated col id. (global_sc_id, local_embedding_id)
    result |= absl::rotr(static_cast<uint32_t>(col_id), num_scs_bit);
    // Shift the combined value to make space for the index.
    result <<= kIndexBits;
    // Add the index to the least significant bits.
    result |= index;
    return result;
  }
};
