...
int ComputeCooBufferSizePerDevice(
  ...
  // only log once per table. We use a static limit of 20 stacked table. There
  // might be a few repeated logs, if we don't have as many tables.
  const int stacked_table_log_count_limit = 20;
  LOG_FIRST_N(INFO, stacked_table_log_count_limit)
  ...
  if (suggested_coo_buffer_size.has_value()) {
    ...
    LOG_FIRST_N(WARNING, stacked_table_log_count_limit)
