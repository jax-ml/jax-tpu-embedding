...
//     Row 1: [3, 4, 5]
//     Row 2: [6]
template <typename T, typename RandomAccessView, typename RandomAccessView_>
class SparseCsrInputBatchStream {
 public:
  SparseCsrInputBatchStream(RandomAccessView values,
                            RandomAccessView_ row_pointers, int row_start,
                            int row_end,
 ...
 private:
  RandomAccessView values_ref_;
  RandomAccessView_ row_pointers_;
  int curr_row_;
  int row_end_;
  int curr_idx_;
  T max_vocab_id_;
...
class UnityWeightsStream {
 public:
  ...
  void seekCol(int col) { curr_col_ = 0; }
...
template <typename T>
UnityWeightsStream(int, int, T) -> UnityWeightsStream<T>;
