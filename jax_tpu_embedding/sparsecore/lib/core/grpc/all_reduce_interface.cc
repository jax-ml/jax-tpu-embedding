...
void GrpcAllReduceInterface::SetUp() {
  for (const auto& peer_address : peer_addresses_) {
    auto creds = ::grpc::Loas2Credentials(::grpc::Loas2CredentialsOptions());
...
absl::StatusOr<AllReduceData> GrpcAllReduceInterface::BlockingAllReduce(
  ...
  absl::Mutex mutex;
  absl::Status overall_status ABSL_GUARDED_BY(mutex) = absl::OkStatus();
  ...
  TF_RETURN_IF_ERROR(overall_status);
