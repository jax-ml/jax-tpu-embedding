...
from jax_tpu_embedding.sparsecore.lib.auto_pipelining import auto_pipelining
...
class ShakespeareTest(absltest.TestCase):
  ...
  def test_shakespeare_model_loss_convergence(self):
    ...
    table_spec = embedding_spec.TableSpec(
        ...
        optimizer=embedding_spec.AdagradOptimizerSpec(learning_rate=1),
        combiner='sum',
        name='shakespeare',
    ...
    def train_step(
      ...
      (loss_val, unused_logits), grads = train_step_fn(
          carry.params, embedding_lookups, labels
      )

      updates, opt_state = tx.update(grads, carry.opt_state)
      params = embed_optimizer.apply_updates_for_sc_model(carry.params, updates)

      return Carry(params, opt_state), loss_val

    step = 0
    carry = Carry(params, opt_state)
    state = auto_pipelining.init_state()
    with XprofHelper(start_step=len(feature_batches) - 5) as xprof_helper:
      for features, labels in zip(feature_batches, label_batches):
        ...
        # Step 2: run model.
        # ----------------------------------------------------------------------
        state, (carry, loss_val) = train_step(
            state, carry, processed_input_tensor, labels
        )
        ...
        logging.info(
            ...
            step,
            loss_val,
            jax.tree.map(jnp.sum, carry.params),
        )

        step += 1

    # Finalize the pipelining run and get the final params and results.
    carry, loss_val = auto_pipelining.finalize_pipelining(
        train_step, state, carry
    )
    logging.info(
        'Final results: loss=%s, params is %s',
        loss_val,
        jax.tree.map(jnp.sum, carry.params),
    )
