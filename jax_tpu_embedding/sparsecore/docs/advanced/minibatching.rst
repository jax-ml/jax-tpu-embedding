...
on-chip memory constraints. Specifically, SparseCore hardware has limits on the
number of embedding IDs (``max_ids_per_partition``) and unique embedding IDs
(``max_unique_ids_per_partition``) that can be processed for a single table
partition in one operation, constraints dictated by the size of its on-chip
memory (SPMEM). When an input batch requires more lookups for a given partition
than these limits allow, minibatching breaks it down into smaller pieces, called
...
the original batch. This mechanism prevents errors caused by exceeding hardware
ID limits (i.e., ``max_ids_per_partition`` and ``max_unique_ids_per_partition``),
and allows users to train with larger logical batch sizes, even if parts of the
input data are highly skewed (i.e., some samples require many more embedding ID
lookups than others). Minibatching is designed to be mathematically equivalent
to processing the full batch in a single pass, and therefore does not affect
model quality.
  ...
  digraph G {
    node [shape=box, style="rounded,filled", fillcolor=white, fontsize=10];
      ...
      subgraph cluster_host_i {
          ...
          i_WaitRPCs [label="2c. WaitIncomingRPCs\n(Last contributing thread waits for N-1 contributions)"];
          i_WaitRes [label="3. WaitResults\n(K-thread barrier)"];
          i_GetRes [label="4. GetResult\n(K threads read result)"];

          i_Init -> i_Send -> i_WaitRPCs -> i_WaitRes -> i_GetRes;
          i_Handler -> i_WaitRPCs [style=dashed, label="unblocks wait"];
      }
      ...
      subgraph cluster_host_j {
          ...
          j_WaitRPCs [label="2c. WaitIncomingRPCs\n(Last contributing thread waits for N-1 contributions)"];
          j_WaitRes [label="3. WaitResults\n(K-thread barrier)"];
          j_GetRes [label="4. GetResult\n(K threads read result)"];

          j_Init -> j_Send -> j_WaitRPCs -> j_WaitRes -> j_GetRes;
          j_Handler -> j_WaitRPCs [style=dashed, label="unblocks wait"];
      }
      ...
      {rank=same; i_WaitRPCs; j_WaitRPCs;}
      {rank=same; i_WaitRes; j_WaitRes;}
...
* **2b. ContributeData**: This is an RPC handler running on Host ``i``'s gRPC
  ...
  ``i``'s state via an OR-reduction and decrements a counter on which
  ``WaitIncomingRPCs`` is waiting.
* **Parallelism**: Because ``SendLocalData`` sends RPCs asynchronously, and
  ...
  from Host ``k``.
* **2c. WaitIncomingRPCs**: After initiating sends via ``SendLocalData``, the
  last contributing thread immediately calls ``WaitIncomingRPCs``, which blocks
  until the ``ContributeData`` handler has been called ``N-1`` times (once for each
  peer), signifying that all peer data has been received and reduced.

Stages 3 and 4 (``WaitResults`` and ``GetResult``) serve as a barrier for all K
local threads to wait for global reduction to complete before retrieving the
final result.
