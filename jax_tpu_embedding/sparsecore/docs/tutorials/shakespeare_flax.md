...
The training step is mostly a call to `model.apply()` and a loss calculation. All the details of the embedding lookup and weight update are handled by the [SparseCoreEmbed](#jax_tpu_embedding.sparsecore.lib.flax.linen.embed.SparseCoreEmbed) Flax layer.
...
implementations or create their own that integrates with their infrastructure.
