...
*   **Action Pinning**: Per Google Open Source security policy, non-Google
    ...
    [Ratchet](https://github.com/sethvarinternal link:ratchet) to pin specific versions. If
    you'd like to update or add an action, you can write something like `uses:
...
*   **Fork PR Protection**: Self-hosted Cloud TPU VM runners
    ...
    prevent untrusted code execution from arbitrary forks, all TPU workflows
    MUST include the security guard:
